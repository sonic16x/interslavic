...
function getCyrillic(text, flavorisationType): string {
    ...
    if ( flavorisationType === '3NC') {
        return transliterate(text, 5, 3, 0, 1)
            .replace(/[ьј]а/g, 'я').replace(/Ја/g, 'Я')
            .replace(/[ьј]е/g, 'є').replace(/Је/g, 'Є')
            .replace(/[ьј]у/g, 'ю').replace(/Ју/g, 'Ю')
            .replace(/ј/g, 'й').replace(/Ј/g, 'Й')
            .replace(/ѣ/g, 'ӗ').replace(/Ѣ/g, 'Ӗ');
    }
